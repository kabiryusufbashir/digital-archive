...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <div id="loginContainer">
            <!-- Login Div  -->
            <div class="sm:w-1/3 sm:mx-auto relative top-24 py-8 px-5 bg-white rounded shadow-md">
                ...
                <div class="px-6">
                    <form action="{{ route('login') }}" method="POST">
                        @csrf
                        <div class="my-2">
                            <input type="text" name="username" placeholder="Username" class="border-gray-300 rounded py-3 px-6 w-full my-2 border-2 border-b focus:outline-none @error('username')  @enderror" autofocus>
                            @error('username')
                                {{$message}}
                            @enderror
                        </div>
                        <div class="my-2">
                            <input type="password" name="password" placeholder="Password" class="border-gray-300 rounded py-3 px-6 w-full my-2 border-2 border-b focus:outline-none @error('password')  @enderror">
                            @error('password')
                                {{$message}}
                            @enderror
                        </div>
